...
import { ADMIN_COOKIE, isDevPasswordBypass, SESSION_DAYS, signSession } from '@/lib/auth'
...
export async function login(_prev: { error: string } | undefined, formData: FormData) {
  ...
  const match =
    isDevPasswordBypass() || (expected.length > 0 && a.length === b.length && timingSafeEqual(a, b))
